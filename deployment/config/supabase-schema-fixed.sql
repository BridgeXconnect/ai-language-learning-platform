-- AI Language Learning Platform Database Schema (Fixed Order)
...
-- Courses table (depends on users)
...
-- Course requests table (depends on users)
...
-- AI chat sessions table (depends on users and courses)
...
-- Lessons table (depends on courses)
...
-- Assessments table (depends on courses)
...
-- User progress table (depends on users, lessons, AND courses)
...
-- AI chat messages table (depends on ai_chat_sessions)
...
-- Assessment questions table (depends on assessments)
...
-- User assessment results table (depends on users and assessments)
...
-- ========================================
-- PHASE 5: Indexes and Performance
-- ========================================
...
SELECT 
    table_name,
    CASE 
        WHEN table_name IN ('users', 'courses', 'course_requests', 'lessons', 'user_progress', 'ai_chat_sessions', 'ai_chat_messages', 'assessments', 'assessment_questions', 'user_assessment_results') 
        THEN '✅ Created'
        ELSE '❌ Missing'
    END as status
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('users', 'courses', 'course_requests', 'lessons', 'user_progress', 'ai_chat_sessions', 'ai_chat_messages', 'assessments', 'assessment_questions', 'user_assessment_results')
ORDER BY table_name; 